...
Use **`AZURE_DEVOPS_PAT`**. Set **Work Items** to **Read & write** in the PAT dialog (labels vary by Azure DevOps version). That covers **`sync`** (read/update/create/comment) and **`azure-devops-smoke`**. When **`auto_create_area_path`** is enabled, **`sync`** only auto-creates the **fallback** area path (not configured CSV/YAML paths); additional project **Create child nodes** permissions may be required — see **[CONFIGURATION.md § Optional — auto-create area paths](CONFIGURATION.md#optional--auto-create-area-paths)**.
...
- **Repo-level ADO routing:** Co-locate **`repo-mapping.csv`** beside your YAML (default filename; override with **`azure_boards.repo_mapping_csv`** or **`REPO_MAPPING_CSV_PATH`**). One **`org_mappings`** row (1:1 Snyk org) plus CSV rows can route repositories to **multiple ADO projects** via **ADO Organization** and full **`Project\\Area`** paths. Optional YAML fallback **`azure_boards.defaults.area_path`** and per-row **`org_mappings[].overrides.area_path`** apply when no CSV row matches. When **`auto_create_area_path`** is **`true`**, missing configured paths use the fallback template (**`auto_create_fallback_area_path`**, default **`{project}\Snyk`**) instead of auto-creating CSV segments — see **[CONFIGURATION.md § repo-mapping.csv](CONFIGURATION.md#azure_boardsrepo_mapping_csv)**. Sample CSV: **`data/sample-repo-mapping.csv`**.
- **Multi-project work item taxonomy:** When CSV routes repositories to ADO projects with different work item types or states, define **`azure_boards.ado_targets`** (one entry per **`organization`** / **`project`**) and optionally per-repo overrides in CSV optional taxonomy columns. See **[CONFIGURATION.md § ado_targets](CONFIGURATION.md#azure_boardsado_targets-optional)**.
...
| **`sync` fails with Azure DevOps HTTP 404 on work items batch** | Usually a **stale mapping**: the mapping store still references a work item id that was deleted in Azure DevOps. Current versions use **`errorPolicy=Omit`** on batch prefetch and **self-heal** for **open** findings (recreate + update mapping). Search logs for **`event`:** **`missing_mapped_work_item`** — fields **`prior_work_item_id`**, **`issue_key`**, **`action`** (`recreate` or `skip`). Set **`create_new_work_items: false`** to skip recreation. |
| **Changed `work_item_type` but old tickets unchanged** | **`work_item_type`**, states, and description field settings apply to **new creates** only. Existing mapped work items are **PATCH**ed in place; **`sync`** does not re-type or bulk-recreate them when configuration changes. To migrate type, clear mapping rows or delete old work items and let **open** issues recreate. |

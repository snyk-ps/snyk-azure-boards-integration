# csv-snyk-org-config-generator Specification

## Purpose

Operator CLI (**`scripts/generate_org_mapping_config.py`**) that reads a CSV of Azure DevOps org/project and Snyk org **names**, lists orgs under a Snyk **group** via the REST API, and writes a starter YAML with **`azure_boards.org_mappings`** (**`snyk_org_id`** / **`snyk_org_slug`**) plus commented defaults and mapping-store hints.
## Requirements
